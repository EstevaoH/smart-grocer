...
const CATEGORY_COLORS = ["#10b981", "#3b82f6", "#f59e0b", "#ef4444", "#8b5cf6", "#ec4899", "#06b6d4", "#84cc16", "#f97316", "#6366f1"];
const MONTH_NAMES = ["Jan", "Fev", "Mar", "Abr", "Mai", "Jun", "Jul", "Ago", "Set", "Out", "Nov", "Dez"];
const STORAGE_BUDGET = "smartgrocer_budget_goal";
...
function itemMonthKey(item: ShoppingItem) {
    const d = item.createdAt ? new Date(item.createdAt) : new Date();
    return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
}

function itemDayKey(item: ShoppingItem) {
    const d = item.createdAt ? new Date(item.createdAt) : new Date();
        ...
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                <div className="flex flex-wrap items-end gap-3">
                    ...
                    {/* F5 CSV Export */}
                    <button onClick={exportCSV} className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium bg-gray-100 text-gray-600 hover:bg-emerald-50 hover:text-primary transition-all">
                ...
                <div className="flex items-center justify-between mb-2 gap-2 flex-wrap">
                    ...
                    {!budgetEdit ? (
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <input ref={budgetRef} type="number" min="0" step="0.01" value={budgetInput} onChange={e => setBudgetInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter") saveBudget(); if (e.key === "Escape") setBudgetEdit(false); }} className="w-28 px-2 py-1 text-sm border border-primary rounded-lg outline-none" />
